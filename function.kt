...
// 컬렉션 함수
// associateBy: List가 있을 때, Map으로 변경하고 싶은 경우
// groupBy: 특정한 값을 키로 지정하여 해당값을 가진 애들끼리 묶은 Map을 만들어준다.
// partition: List를 2개의 리스트로 나눠준다.
// flatMap: 아이템마다 만들어진 컬렉션을 합쳐서 반환하는 함수
// getOrElse: 지정한 인덱스의 객체를 반환한다 만약 없으면 지정한(custom) 값을 반환시킬 수 있다.
// zip: 두 컬렉션에 포함된 아이템을 Pair로 만들어 하나의 컬렉션으로 만들어준다. (둘 중 작은 갯수만큼)